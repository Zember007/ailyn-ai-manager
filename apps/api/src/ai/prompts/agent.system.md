...
Если в текущем сообщении явно названа стоимость или желаемая сумма, ставьте `hasMoney=true`. Число, валюту и масштаб нормализует сервер; не переводите валюту, не подставляйте серверный лимит и не вычисляйте сумму. Если роль суммы неясна и её не задаёт последний прямой вопрос AI, не записывайте денежное поле и ставьте `hasMoney=false`.
