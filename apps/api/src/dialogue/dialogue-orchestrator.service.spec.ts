...
import { appendWorkflowFollowUp, DialogueOrchestratorService, composeReply, removeEarlierDuplicateSentences, replaceMaximumLimitPlaceholders, resolveForeignCurrencyFacts, resolveNormalizedMoneyFacts, stripKnowledgeQuestions, stripUnrequestedAssistanceOffers, workflowFollowUpAfterKnowledge } from "./dialogue-orchestrator.service.js";
...
describe("single-agent dialogue", () => {
  it("creates a new application for a repeat loan and requests current collateral without reusing ID", async () => {
    const closedApplication = {
      id: "closed-app",
      contactId: "contact",
      previousApplicationId: "first-closed-app",
      stage: "CLOSED",
    ...
    const newApplication = {
      ...
      previousApplicationId: "closed-app",
      facts: {
        fullName: "Иванов Иван",
        residenceRegion: "Бишкек",
        residenceCategory: "BISHKEK_CHUY",
    ...
    const conversation = { id: "conversation", messages: [{ author: "ai", body: "Ваша заявка оформлена", createdAt: "now" }], application: closedApplication, channel: "web-test" } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application: closedApplication }),
      findLatestClosedApplication: vi.fn().mockResolvedValue(closedApplication),
      createRepeatLoanApplication: vi.fn().mockResolvedValue(newApplication),
      addMessage: vi.fn().mockResolvedValue({ id: "message", createdAt: "now" }),
      updateFacts: vi.fn().mockResolvedValue([]),
      saveAgentState: vi.fn(),
      getApplication: vi.fn().mockResolvedValue(newApplication),
      getConversation: vi.fn().mockResolvedValue({ ...conversation, application: newApplication }),
      addAttachment: vi.fn(),
    ...
    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn(), warn: vi.fn() } as any)
      .receive({ externalMessageId: "repeat", channel: "web-test", externalContactId: "contact", text: "Хочу снова займ под ту же машину", attachments: [], timestamp: new Date() });
    ...
    expect(output.reply).toContain("свидетельство о регистрации ТС с обеих сторон");
    expect(output.reply).toContain("Окончательное решение и сумма определяются после проверки автомобиля и документов.");
    expect(output.reply).not.toMatch(/ФИО|прописан|семейн|ID/iu);
  });

  it("does not create a repeat application for a mixed active-contract service request", async () => {
    const application = { id: "active-app", contactId: "contact", facts: {}, stage: "TARGET_REACHED_VISIT", status: "target_reached" } as any;
    const conversation = { id: "conversation", messages: [], application, channel: "web-test" } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application }),
      findLatestClosedApplication: vi.fn(),
      createRepeatLoanApplication: vi.fn(),
      addMessage: vi.fn().mockResolvedValue({ id: "message", createdAt: "now" }),
      updateFacts: vi.fn().mockResolvedValue([]),
      saveAgentState: vi.fn(),
    ...
    await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn(), warn: vi.fn() } as any)
      .receive({ externalMessageId: "repeat", channel: "web-test", externalContactId: "contact", text: "Хочу снова займ, но у меня есть действующий договор", attachments: [], timestamp: new Date() });
  ...
  it("starts a fresh application when an existing card receives an explicit new-loan request", async () => {
    const application = { id: "active-app", contactId: "contact", stage: "COLLECTING_VEHICLE", status: "need_more_data", facts: { vehicleModel: "Camry", vehicleYear: 2020, vehicleValue: 1_000_000 } } as any;
    const closedApplication = { ...application, stage: "CLOSED", status: "closed" } as any;
    const newApplication = { id: "new-app", contactId: "contact", stage: "NEW", status: "need_more_data", facts: {} } as any;
    const conversation = { id: "conversation", messages: [], application, channel: "web-test" } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application }),
      findLatestClosedApplication: vi.fn().mockResolvedValue(undefined),
    ...
    const agent = { run: vi.fn().mockResolvedValue({ result: { ...validResult, leadCardPatch: {}, attachments: [] }, reply: "Поняла.", model: "one", promptVersion: "v1" }) } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn(), warn: vi.fn() } as any)
      .receive({ externalMessageId: "new", channel: "web-test", externalContactId: "contact", text: "нужен новый займ", attachments: [], timestamp: new Date() });
  ...
  it("removes unrequested assistance offers from a knowledge answer", () => {
    ...
    expect(appendWorkflowFollowUp("Подскажите, пожалуйста, Вашу прописку.", "Подскажите, пожалуйста, Вашу прописку — Бишкек, Чуйская область или другой регион Кыргызстана.")).toBe("Подскажите, пожалуйста, Вашу прописку.");
    expect(appendWorkflowFollowUp(
      "Понимаю, что ситуация может быть неприятной.",
      "Вас интересует займ без изъятия автомобиля или с постановкой автомобиля на охраняемую стоянку?",
      "Обычно оформление занимает около 1 часа. Вас интересует займ без изъятия автомобиля или с постановкой автомобиля на охраняемую стоянку?"
    )).toBe("Понимаю, что ситуация может быть неприятной.");
    const knowledgeLimit = "Вам доступен займ без изъятия автомобиля на сумму до 600 000 сом. Если Вам нужно 700 000 сом, это возможно только по программе со стоянкой на охраняемой парковке. Ставка по этой программе составляет 2,4% в месяц.";
    const serverLimit = "По программе без изъятия доступно до 600 000 сом. Сумма 700 000 сом по этой программе не проходит. Со стоянкой при текущей стоимости автомобиля доступно до 1 000 000 сом. Могу продолжить либо на сумму до 600 000 сом без изъятия, либо перейти на программу со стоянкой и рассмотреть сумму до 1 000 000 сом.";
    expect(appendWorkflowFollowUp(knowledgeLimit, serverLimit)).toBe(serverLimit);
    expect(stripKnowledgeQuestions("Ставка определяется индивидуально после осмотра автомобиля. Какую программу Вы предпочитаете?\nПодскажите, пожалуйста, сумму?")).toBe("Ставка определяется индивидуально после осмотра автомобиля.");
  });

  it("passes the two latest dialogue messages to the main agent for semantic stage matching", async () => {
    ...
    await new AgentTurnService(client).run({
      messages: [
        { author: "client", body: "Фотографии отправил", createdAt: "first" },
        { author: "ai", body: "Фотографии получены. Подскажите, пожалуйста, Ваше семейное положение — Вы в браке, в разводе или не в браке.", createdAt: "second" }
      ] as any,
      facts: { residenceRegion: "Бишкек", residenceCategory: "BISHKEK_CHUY" } as any,
      settings: {}, text: "холост", attachments: []
    });

    const context = JSON.parse(client.createChatCompletion.mock.calls[0][0].messages[1].content[0].text);
    expect(context.lastTwoDialogueMessages).toEqual([
      { author: "client", text: "Фотографии отправил", createdAt: "first" },
      { author: "ai", text: "Фотографии получены. Подскажите, пожалуйста, Ваше семейное положение — Вы в браке, в разводе или не в браке.", createdAt: "second" }
    ]);
  });
  ...
  it("interprets a past-tense marriage answer as divorced", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...validResult,
      reply: "Для оформления потребуется нотариальное согласие супруга или супруги.",
      leadCardPatch: { familyStatus: "married" }
    }) } }] }) } as any;
    ...
    const output = await new AgentTurnService(client).run({
      ...
      facts: {
        vehicleModel: "Camry", vehicleYear: 2020, vehicleValue: 1_000_000,
      ...
      settings: {}, text: "В браке был", attachments: []
    });

    expect(output.result?.leadCardPatch.familyStatus).toBe("divorced");
    expect(output.reply).toMatch(/автомобил\p{L}*.*(?:во время брака|после развода)/iu);
    expect(output.reply).not.toMatch(/нотариальн|согласие\s+супруг/iu);
  });
  ...
  it("treats a first-turn parking selection as data, not an unasked programme FAQ", async () => {
    ...
    expect(output.reply).toMatch(/ориентировочн\p{L}*\s+стоимост|модел\p{L}*.*год\p{L}*\s+выпуск/iu);
    expect(output.reply).not.toMatch(/ставка|парковк|охраняем/iu);
  });

  it("treats an accepting parking reply as the selected programme when the program model misses it", async () => {
    const client = {
      isConfigured: vi.fn().mockReturnValue(true),
      createChatCompletion: vi.fn()
        .mockResolvedValueOnce({ choices: [{ message: { content: JSON.stringify({
          ...validResult,
          reply: "По программе со стоянкой ставка составляет 2,4% в месяц.",
          leadCardPatch: {}
        }) } }] })
        .mockResolvedValueOnce({ choices: [{ message: { content: JSON.stringify({
          program: null, hasOtherStageAnswer: false, question: null
        }) } }] })
    } as any;
    ...
    const output = await new AgentTurnService(client).run({
      ...
      facts: { vehicleModel: "Camry", vehicleYear: 2020, vehicleValue: 1_000_000, requestedAmount: 300_000, residenceRegion: "Бишкек", residenceCategory: "BISHKEK_CHUY" } as any,
      settings: {}, text: "стоянка устроит", attachments: []
    });
  ...
  it("does not deliver a KB rate when the client only selects a programme", async () => {
    const application = {
    ...
    const conversation = {
      ...
      application,
      channel: "web-test",
      messages: [{ author: "ai", body: "Вас интересует займ без изъятия автомобиля или с постановкой автомобиля на охраняемую стоянку?", createdAt: "now" }]
    } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application }),
      addMessage: vi.fn().mockImplementation(async (_conversation: unknown, message: any) => ({ id: message.metadata.externalMessageId, author: message.author, body: message.body, createdAt: "now" })),
      updateFacts: vi.fn().mockResolvedValue(["requestedProgram"]), saveAgentState: vi.fn(), getApplication: vi.fn().mockResolvedValue(application), getConversation: vi.fn().mockResolvedValue(conversation), addAttachment: vi.fn(), createManagerNotification: vi.fn()
    } as any;
    const selectedReply = "Пожалуйста, отправьте фото ID и свидетельства о регистрации автомобиля с обеих сторон.";
    const agent = {
      run: vi.fn().mockResolvedValue({ result: { ...validResult, reply: selectedReply, leadCardPatch: { requestedProgram: "parking" } }, reply: selectedReply, model: "one", promptVersion: "v1" }),
      answerWithKnowledge: vi.fn().mockResolvedValue({ reply: "По программе со стоянкой ставка составляет 2,4% в месяц.", answerFound: true, shouldUseReply: true, requestScope: "new_loan", model: "knowledge" })
    } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "parking", channel: "web-test", externalContactId: "contact", text: "со стоянкой", attachments: [], timestamp: new Date() });
  ...
  it("detects a future year from a terse reply to the active year question even when the model omits it", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...validResult,
      reply: "Подскажите, пожалуйста, год выпуска автомобиля.",
      leadCardPatch: {}
    }) } }] }) } as any;
    ...
    const output = await new AgentTurnService(client).run({
      ...
      settings: {}, text: "2029", attachments: []
    });

    expect(output.result?.leadCardPatch).toMatchObject({ reportedInvalidVehicleYear: 2029 });
    expect(output.reply).toBe("2029 год ещё не наступил. Уточните, пожалуйста, верный год выпуска автомобиля.");
    expect(output.result?.leadCardPatch.vehicleYear).toBeUndefined();
  });
  ...
  it("passes a short follow-up after the UNA rule to the model with its prior context", async () => {
    ...
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({ reply: answer, answerFound: true, contextualPolicyRelation: "follow_up" }) } }] }) } as any;
    const output = await new AgentTurnService(client).answerWithKnowledge({
  ...
  it("lets the model answer an UNA follow-up without a server-written response", async () => {
    const answer = "Без регистрации автомобиля в УНА оформить займ нельзя: сначала нужно поставить его на учёт.";
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...validResult,
      reply: answer,
  ...
  it("keeps the KB model authoritative for guarantor classification", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
    ...
    expect(output).toMatchObject({ answerFound: true, shouldUseReply: true });
  });

  it("uses only the KB answer for a guarantor question without appending a server duplicate", async () => {
    const reply = "Да, в Вашем случае потребуется поручитель.";
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply, answerFound: true, questionUnderstood: true, sourceKeys: ["lead_card"], requestScope: "new_loan", contextualPolicyRelation: null
    }) } }] }) } as any;

    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [], facts: { requestedProgram: "without_storage", residenceCategory: "OTHER_KG" } as any,
      settings: {}, text: "а нужен поручитель будет мне?", workflowFollowUp: ""
    });

    expect(output?.reply).toBe(reply);
    expect(output?.reply.match(/поручител/giu)).toHaveLength(1);
  });

  it("delivers a KB guarantor answer for an unpunctuated natural question", async () => {
    const reply = "Да, в Вашем случае потребуется поручитель.";
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply, answerFound: true, questionUnderstood: true, sourceKeys: ["faq_guarantor_requirement"], requestScope: "new_loan", contextualPolicyRelation: null
    }) } }] }) } as any;

    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [], facts: { requestedProgram: "without_storage", residenceCategory: "OTHER_KG" } as any,
      settings: {}, text: "А поручителя искать", workflowFollowUp: ""
    });

    expect(output).toMatchObject({ reply, answerFound: true, shouldUseReply: true });
  });

  it("reduces a verbose KB guarantor explanation to the direct answer", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply: "Поручитель нужен только при займе без изъятия автомобиля для клиентов, зарегистрированных за пределами Бишкека и Чуйской области. В Вашем случае, поскольку прописка указана как \"Другой регион Кыргызстана\", поручитель потребуется.",
      answerFound: true, questionUnderstood: true, sourceKeys: ["faq_guarantor_requirement"], requestScope: "new_loan", contextualPolicyRelation: null
    }) } }] }) } as any;

    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [], facts: { requestedProgram: "without_storage", residenceCategory: "OTHER_KG" } as any,
      settings: {}, text: "А поручитель нужен?", workflowFollowUp: ""
    });

    expect(output?.reply).toBe("Да, в Вашем случае потребуется поручитель.");
  });

  it("does not turn a residence fact into an unsolicited guarantor answer", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply: "Поручитель потребуется только для программы без изъятия при прописке за пределами Бишкека и Чуйской области.",
      answerFound: true, questionUnderstood: true, sourceKeys: ["faq_guarantor_requirement"], requestScope: "new_loan", contextualPolicyRelation: null
    }) } }] }) } as any;

    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [{ author: "ai", body: "Подскажите, пожалуйста, Вашу прописку.", createdAt: "now" } as any],
      facts: {}, settings: {}, text: "Я в чуйске прописан", workflowFollowUp: "Подскажите, пожалуйста, Вашу прописку."
    });

    expect(output).toMatchObject({ reply: "__WORKFLOW_STAGE_RESPONSE__", answerFound: false, shouldUseReply: false });
  });
  ...
  it("keeps an inspection question when a known-guarantor answer shares the visit turn", async () => {
    ...
    const output = await new AgentTurnService(client).answerWithKnowledge({
      ...
      facts: { requestedProgram: "parking", residenceRegion: "Бишкек", residenceCategory: "BISHKEK_CHUY" } as any,
      settings: {}, text: "Понял, а как у вас осмотр проводится, мне надо поручителя искать? Я приеду наверное завтра в 6", workflowFollowUp: ""
    });

    expect(client.createChatCompletion).toHaveBeenCalledOnce();
    expect(output?.answerFound).toBe(true);
    expect(output?.reply).toContain("Осмотр автомобиля проводится");
    expect(output?.reply).not.toContain("поручитель не требуется");
  });

  it("records tomorrow at six from a mixed inspection and guarantor message", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
    ...
    const output = await new AgentTurnService(client).run({
      messages: [{ author: "ai", body: "Офис работает с понедельника по пятницу с 11:00 до 19:00. Для оформления нужно приехать не позднее 18:00. На какой день и время Вам удобно подъехать?", createdAt: "now" } as any],
      facts: {
        vehicleModel: "Camry", vehicleYear: 2022, vehicleValue: 3_000_000, requestedAmount: 600_000,
        requestedProgram: "parking", residenceRegion: "Бишкек", residenceCategory: "BISHKEK_CHUY",
        documentsProvided: true, documents: { car_photo: "received" }, familyStatus: "divorced", vehicleBoughtDuringMarriage: false
      } as any,
      settings: {}, text: "Понял, а как у вас осмотр проводится, мне надо поручителя искать? Я приеду наверное завтра в 6", attachments: []
    });
  ...
  it("marks a GPS malfunction as existing-loan servicing for the knowledge model", async () => {
    ...
    const output = await service.answerWithKnowledge({ messages: [], facts: {}, settings: {}, text: "у меня отвалился трекер мне надо к вам приехать?", workflowFollowUp: "" });

    const context = JSON.parse(client.createChatCompletion.mock.calls[0][0].messages[1].content);
    expect(output).toMatchObject({ answerFound: true, reply: "Если у Вас уже оформлен займ, пожалуйста, позвоните по телефону +996 502 108 108 или напишите в WhatsApp +996 776 108 108." });
    expect(context.existingContractServiceRequest).toBe(true);
    expect(context.knowledge.some((chunk: { section?: string }) => chunk.section === "3.18")).toBe(true);
    expect(context.knowledge.some((chunk: { key?: string }) => chunk.key === "faq_gps_requirement")).toBe(true);
  });
  ...
  it("does not attach an independent rate question to the preceding UNA policy", async () => {
    ...
    // KB starts speculatively with the workflow model. The terminal refusal
    // still owns delivery once the workflow result is available.
    expect(agent.answerWithKnowledge).toHaveBeenCalledOnce();
  });
  ...
  it("moves from a maximum request at the amount stage to residence when residence is missing", async () => {
    ...
    expect(output.reply).toBe("Максимальную сумму смогу рассчитать после того, как узнаю: Вашу прописку.\n\nПодскажите, пожалуйста, Вашу прописку — Бишкек, Чуйская область или другой регион Кыргызстана.");
    expect(output.reply).not.toContain(amountStageQuestion);
  ...
  it("keeps only the residence stage after a maximum answer when the workflow model echoed vehicle collection", async () => {
    ...
    expect(agent.answerWithKnowledge).toHaveBeenCalledWith(expect.objectContaining({ workflowFollowUp: amountStageQuestion }));
  ...
  it("answers the deferred maximum request immediately after the missing residence is supplied", async () => {
    ...
    const agent = {
      run: vi.fn().mockResolvedValue({
        // The workflow model normally returns only the newly recognized
        // residence. Vehicle facts are already persisted on the application.
        result: { ...validResult, leadCardPatch: { residenceText: resolvedFacts.residenceText, residenceRegion: resolvedFacts.residenceRegion, residenceCategory: resolvedFacts.residenceCategory } },
        reply: "Поняла.", model: "workflow-model", promptVersion: "v1"
      }),
      answerWithKnowledge: vi.fn().mockResolvedValue({
        reply: "Без изъятия: от 50 000 сом до MAX_LIMIT_WITHOUT сом\nСо стоянкой: от 50 000 сом до MAX_LIMIT_PARK сом",
    ...
    expect(agent.answerWithKnowledge).toHaveBeenCalledWith(expect.objectContaining({ text: "Бишкек", workflowFollowUp: amountStageQuestion }));
    expect(output.reply).toBe("Без изъятия: от 50 000 сом до 600 000 сом\nСо стоянкой: от 50 000 сом до 1 500 000 сом");
  });
  ...
  it("does not let KB replace a server terminal refusal with a repeated visit question", async () => {
    const application = { id: "app", facts: {}, contactId: "contact", stage: "REFUSED", status: "refuse" } as any;
    const conversation = { id: "conversation", messages: [], application, channel: "web-test" } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application }),
      addMessage: vi.fn().mockImplementation(async (_conversation: unknown, message: any) => ({ id: message.metadata.externalMessageId, author: message.author, body: message.body, createdAt: "now" })),
    ...
    const serverRefusal = "К сожалению, мы не можем оформить займ под автомобиль, зарегистрированный в 10 регионе.";
    const repeatedKnowledgeReply = `${serverRefusal} Офис работает с понедельника по пятницу с 11:00 до 19:00. На какой день и время Вам удобно подъехать?`;
    const agent = {
      run: vi.fn().mockResolvedValue({
        result: { ...validResult, reply: serverRefusal, leadCardPatch: { vehicleRegistrationRegion: "10" }, dialogueState: { stage: "REFUSED", status: "refuse", nextAction: "refuse" }, needsKnowledgeLookup: true },
        reply: serverRefusal, model: "workflow-model", promptVersion: "v1"
      }),
      answerWithKnowledge: vi.fn().mockResolvedValue({ reply: repeatedKnowledgeReply, answerFound: true, shouldUseReply: true, model: "knowledge-model" })
    } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "m", channel: "web-test", externalContactId: "c", text: "У меня авто в 10 регионе", attachments: [], timestamp: new Date() });

    // KB starts speculatively with the workflow model. The terminal refusal
    // remains a server-owned reply and is never replaced by that result.
    expect(agent.answerWithKnowledge).toHaveBeenCalledOnce();
    expect(output.reply).toBe(serverRefusal);
    expect(output.reply).not.toContain("На какой день и время");
  });

  it("does not claim a non-empty reply was unread when the client says they already wrote it", async () => {
    ...
    const application = {
      id: "app",
      facts: { vehicleMake: "Toyota", vehicleModel: "Camry", vehicleYear: 2020, vehicleValue: 1_500_000 },
      contactId: "contact",
    ...
    const conversation = {
      id: "conversation",
      messages: [{ author: "ai", body: amountStageQuestion, createdAt: "now" }],
      application,
      channel: "web-test"
    } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application }),
      addMessage: vi.fn().mockResolvedValue({ id: "message", author: "client", body: "Я уже писал", createdAt: "now" }),
      updateFacts: vi.fn().mockResolvedValue([]), saveAgentState: vi.fn(), getApplication: vi.fn().mockResolvedValue(application), getConversation: vi.fn().mockResolvedValue(conversation), addAttachment: vi.fn(), createManagerNotification: vi.fn()
    } as any;
    const agent = {
      run: vi.fn().mockResolvedValue({
      ...
      answerWithKnowledge: vi.fn().mockResolvedValue({ reply: redirect, answerFound: true, shouldUseReply: true, model: "knowledge-model" })
    } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "m", channel: "web-test", externalContactId: "c", text: "Я уже писал", attachments: [], timestamp: new Date() });

    expect(output.reply).toBe("Поняла.");
    expect(output.reply).not.toContain("Какая сумма займа Вам необходима?");
    expect(output.reply).not.toContain("Я Вас не совсем поняла");
    ...
    expect(store.saveAgentState).toHaveBeenCalledWith(application, expect.objectContaining({
      stage: "COLLECTING_AMOUNT", status: "need_more_data", nextAction: "continue_application"
    }));
  });

  it("asks for detail only when an empty message leaves the same stage unresolved", async () => {
    const application = {
      id: "app", contactId: "contact", stage: "COLLECTING_AMOUNT", status: "need_more_data",
      facts: { vehicleModel: "Camry", vehicleYear: 2020, vehicleValue: 1_500_000 }
    } as any;
    const conversation = {
      id: "conversation", application, channel: "web-test",
      messages: [{ author: "ai", body: amountStageQuestion, createdAt: "now" }]
    ...
    const agent = {
      run: vi.fn().mockResolvedValue({ result: { ...validResult, reply: amountStageQuestion, leadCardPatch: {} }, reply: amountStageQuestion, model: "workflow-model", promptVersion: "v1" }),
      answerWithKnowledge: vi.fn().mockResolvedValue({ reply: "__WORKFLOW_STAGE_RESPONSE__", answerFound: false, shouldUseReply: false, model: "knowledge-model" })
    } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "unresolved", channel: "web-test", externalContactId: "contact", text: "", attachments: [], timestamp: new Date() });

    expect(output.reply).toBe("Я Вас не совсем поняла. Можете, пожалуйста, написать подробнее?");
    expect(output.reply).not.toContain(amountStageQuestion);
  });

  it("continues with the next stage after residence was recorded instead of calling it unclear", async () => {
    const residenceQuestion = "Подскажите, пожалуйста, Вашу прописку — Бишкек, Чуйская область или другой регион Кыргызстана?";
    const application = {
      id: "app", contactId: "contact", stage: "COLLECTING_RESIDENCE", status: "need_more_data",
      facts: { vehicleModel: "Camry", vehicleYear: 2020, vehicleValue: 1_500_000 }
    } as any;
    const conversation = {
      id: "conversation", application, channel: "web-test",
      // Legacy/model output may contain two prompts. Its final amount wording
      // must not make the next legitimate amount question look like a retry.
      messages: [{ author: "ai", body: `Максимальную сумму смогу рассчитать после прописки. ${residenceQuestion} ${amountStageQuestion}`, createdAt: "now" }]
    } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application }),
      addMessage: vi.fn().mockResolvedValue({ id: "message", author: "client", body: "Чуйская область", createdAt: "now" }),
      updateFacts: vi.fn().mockResolvedValue(["residenceText", "residenceRegion", "residenceCategory"]), saveAgentState: vi.fn(), getApplication: vi.fn().mockResolvedValue(application), getConversation: vi.fn().mockResolvedValue(conversation), addAttachment: vi.fn(), createManagerNotification: vi.fn()
    } as any;
    const agent = {
      run: vi.fn().mockResolvedValue({
        result: { ...validResult, reply: amountStageQuestion, currentStageResponse: "answer", leadCardPatch: { residenceText: "Чуйская область", residenceRegion: "Чуйская область", residenceCategory: "BISHKEK_CHUY", residenceNeedsClarification: false } },
        reply: amountStageQuestion, model: "workflow-model", promptVersion: "v1"
      }),
      answerWithKnowledge: vi.fn().mockResolvedValue({ reply: "__WORKFLOW_STAGE_RESPONSE__", answerFound: false, shouldUseReply: false, model: "knowledge-model" })
    } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "residence", channel: "web-test", externalContactId: "contact", text: "Чуйская область", attachments: [], timestamp: new Date() });

    expect(output.reply).toBe(amountStageQuestion);
    expect(output.reply).not.toContain("Я Вас не совсем поняла");
  });

  it("keeps only the current residence question when a maximum-response plan contains two stages", async () => {
    const residenceQuestion = "Подскажите, пожалуйста, Вашу прописку — Бишкек, Чуйская область или другой регион Кыргызстана?";
    const application = { id: "app", contactId: "contact", stage: "COLLECTING_RESIDENCE", status: "need_more_data", facts: { vehicleModel: "Camry", vehicleYear: 2020, vehicleValue: 1_500_000 } } as any;
    const conversation = { id: "conversation", application, channel: "web-test", messages: [] } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application }),
      addMessage: vi.fn().mockResolvedValue({ id: "message", author: "client", body: "сколько максимум", createdAt: "now" }),
      updateFacts: vi.fn().mockResolvedValue([]), saveAgentState: vi.fn(), getApplication: vi.fn().mockResolvedValue(application), getConversation: vi.fn().mockResolvedValue(conversation), addAttachment: vi.fn(), createManagerNotification: vi.fn()
    } as any;
    const replyWithTwoStages = `Максимальную сумму смогу рассчитать после прописки. ${residenceQuestion} ${amountStageQuestion}`;
    const agent = {
      run: vi.fn().mockResolvedValue({ result: { ...validResult, reply: replyWithTwoStages, leadCardPatch: {} }, reply: replyWithTwoStages, model: "workflow-model", promptVersion: "v1" }),
      answerWithKnowledge: vi.fn().mockResolvedValue({ reply: "__WORKFLOW_STAGE_RESPONSE__", answerFound: false, shouldUseReply: false, model: "knowledge-model" })
    } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "maximum", channel: "web-test", externalContactId: "contact", text: "сколько максимум", attachments: [], timestamp: new Date() });

    expect(output.reply).toContain(residenceQuestion);
    expect(output.reply).not.toContain(amountStageQuestion);
    expect(output.reply.match(/[?？]/gu)).toHaveLength(1);
  });

  it("keeps the server visit confirmation instead of replacing it with a KB answer", async () => {
    const application = { id: "app", facts: {}, contactId: "contact", stage: "SCHEDULING_VISIT", status: "need_more_data" } as any;
    const conversation = { id: "conversation", messages: [], application, channel: "web-test" } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application }),
    ...
    const serverConfirmation = "Записываю Вас на четверг, 17.09, в 17:00.\nАдрес: Б. Молодой Гвардии, 22, Бишкек\n2ГИС: https://go.2gis.com/Y34m4\nGoogle Maps: https://maps.app.goo.gl/9xiWLVvdyRgn3Sx4A\n\nЗапись предварительная, её подтвердит менеджер.";
    const agent = {
      run: vi.fn().mockResolvedValue({
        result: { ...validResult, reply: serverConfirmation, leadCardPatch: { visitRequested: true, visitDate: "2026-09-17", visitTime: "17:00" }, dialogueState: { stage: "SCHEDULING_VISIT", status: "need_more_data", nextAction: "schedule_visit" } },
        reply: serverConfirmation, model: "workflow-model", promptVersion: "v1"
      }),
      answerWithKnowledge: vi.fn().mockResolvedValue({ reply: "По нашим правилам запись оформляется в офисе.", answerFound: true, shouldUseReply: true, model: "knowledge-model" })
    } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "m", channel: "web-test", externalContactId: "c", text: "завтра в 5", attachments: [], timestamp: new Date() });

    // KB starts speculatively with the workflow model. The confirmed visit
    // remains a server-owned reply and is never replaced by that result.
    expect(agent.answerWithKnowledge).toHaveBeenCalledOnce();
    expect(output.reply).toBe(serverConfirmation);
    expect(output.reply).toContain("Адрес: Б. Молодой Гвардии, 22, Бишкек");
    expect(output.reply).toContain("Запись предварительная, её подтвердит менеджер");
  });

  it("keeps both the KB answer and booking confirmation when a visit message has a separate question", async () => {
    const application = { id: "app", facts: {}, contactId: "contact", stage: "SCHEDULING_VISIT", status: "need_more_data" } as any;
    const conversation = { id: "conversation", messages: [], application, channel: "web-test" } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application }),
    ...
    const booking = "Записываю Вас на четверг, 17.09, в 18:00.\nАдрес: Б. Молодой Гвардии, 22, Бишкек\n\nЗапись предварительная, её подтвердит менеджер.";
    const agent = {
      run: vi.fn().mockResolvedValue({
        result: { ...validResult, reply: booking, leadCardPatch: { visitRequested: true, visitDate: "2026-09-17", visitTime: "18:00" }, dialogueState: { stage: "SCHEDULING_VISIT", status: "need_more_data", nextAction: "schedule_visit" } },
        reply: booking, model: "workflow-model", promptVersion: "v1"
      }),
      answerWithKnowledge: vi.fn().mockResolvedValue({ reply: "Осмотр автомобиля проводится в офисе. Нет, в Вашем случае поручитель не требуется.", answerFound: true, shouldUseReply: true, model: "knowledge-model" })
    } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "m", channel: "web-test", externalContactId: "c", text: "Как проводится осмотр, поручителя искать? Я приеду завтра в 6", attachments: [], timestamp: new Date() });

    expect(output.reply).toContain("Осмотр автомобиля проводится в офисе.");
    expect(output.reply).toContain("поручитель не требуется");
    expect(output.reply).toContain("Записываю Вас на четверг");
  });
  ...
  it("tells the money normalizer to correct a typo in a loan amount suffix", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({ values: [
    ...
    const result = await new AgentTurnService(client).normalizeMoney({
      text: "сузуки лифан нужно 1 слн стоит 3 млн примерно", facts: {}, messages: []
    });

    expect(result).toEqual(expect.arrayContaining([
      expect.objectContaining({ field: "requestedAmount", amount: 1_000_000 }),
      expect.objectContaining({ field: "vehicleValue", amount: 3_000_000 })
    ]));
    expect(client.createChatCompletion.mock.calls[0]?.[0].messages[0].content).toContain("1 слн");
  });
  ...
  it("records a stated requested amount and asks for the missing vehicle fact", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...validResult,
      hasMoney: true,
    ...
    expect(output.result?.loanQuestionKind).toBe("none");
    expect(output.result?.leadCardPatch.requestedAmount).toBe(1_000_000);
    expect(output.reply).toMatch(/ориентировочн\p{L}* стоимость автомобил/iu);
    expect(output.reply).not.toContain("Максимальную сумму смогу рассчитать");
  });
  ...
  it("keeps a stated requested amount while asking for the earliest remaining stage", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...validResult,
      hasMoney: true,
    ...
    expect(output.result?.leadCardPatch.requestedAmount).toBe(1_000_000);
    expect(output.reply).toMatch(/ориентировочн\p{L}* стоимость автомобил/iu);
    expect(output.reply).not.toContain("Предварительный диапазон займа");
  });
  ...
  it("uses only the server maximum template when a price correction shares a maximum request", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply: "К сожалению, мы не сможем оформить займ, если стоимость автомобиля указана неверно. Если стоимость автомобиля 5 миллионов сом, то займ можно оформить только с изъятием, и максимальная сумма составит 2 миллиона сом.",
      answerFound: true,
      questionUnderstood: true,
    ...
    const reply = await new AgentTurnService(client).answerWithKnowledge({
      messages: [],
      facts: { vehicleValue: 5_000_000, residenceRegion: "Бишкек", residenceCategory: "BISHKEK_CHUY" } as any,
      settings: {},
  ...
  it("keeps a grounded raw answer when the model misspells its FAQ source key", async () => {
    const rawReply = "Да, мы выдаем займ без изъятия автомобиля. Ставка определяется индивидуально после осмотра автомобиля и проверки документов.";
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply: rawReply,
      answerFound: true,
      questionUnderstood: true,
      sourceKeys: ["faq_without_seization_rate"],
      requestScope: "new_loan",
      contextualPolicyRelation: null
    }) } }] }) } as any;

    const reply = await new AgentTurnService(client).answerWithKnowledge({
      messages: [], facts: {}, settings: {}, text: "Выдаете займ без изъятия?", workflowFollowUp: ""
    });

    expect(reply).toMatchObject({ reply: rawReply, answerFound: true, shouldUseReply: true });
  });

  it("does not replace a raw value-correction answer merely because it cites the maximum FAQ", async () => {
    const rawReply = "Если стоимость автомобиля была указана неверно, это может повлиять на условия займа. Окончательная сумма займа определяется после осмотра автомобиля и проверки документов.";
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply: rawReply,
      answerFound: true,
      questionUnderstood: true,
    ...
    const reply = await new AgentTurnService(client).answerWithKnowledge({
      messages: [], facts: {}, settings: {}, text: "круто, а если я стоймость авто перепутал, это повлияет на условия", workflowFollowUp: ""
    });

    expect(reply).toMatchObject({ reply: rawReply, answerFound: true, shouldUseReply: true });
    expect(reply?.reply).not.toContain("MAX_LIMIT");
  });
  ...
  it("treats 'чем больше, тем лучше' after the amount stage as a maximum-limit request", async () => {
    ...
    const reply = await new AgentTurnService(client).answerWithKnowledge({
      ...
      workflowFollowUp: "Какая сумма займа Вам необходима?"
    });
    ...
    const context = JSON.parse(client.createChatCompletion.mock.calls[0][0].messages[1].content);
    expect(context.maximumLoanTemplate).toBeTruthy();
    expect(client.createChatCompletion.mock.calls[0][0].messages[0].content).toContain("Исключение для максимальной суммы");
  });

  it("delivers a confirmed KB maximum answer even when the workflow model labels the turn as a stage answer", async () => {
    const application = {
      id: "app", contactId: "contact", stage: "COLLECTING_AMOUNT", status: "need_more_data",
      facts: { vehicleModel: "Camry", vehicleYear: 2020, vehicleValue: 3_000_000, residenceRegion: "Бишкек", residenceCategory: "BISHKEK_CHUY" }
    } as any;
    const conversation = {
      id: "conversation", application, channel: "web-test",
      messages: [{ author: "ai", body: "Я Вас не совсем поняла. Можете, пожалуйста, написать подробнее?", createdAt: "now" }]
    } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application }),
      addMessage: vi.fn().mockResolvedValue({ id: "message", author: "client", body: "чем больше, тем лучше", createdAt: "now" }),
      updateFacts: vi.fn().mockResolvedValue([]), saveAgentState: vi.fn(), getApplication: vi.fn().mockResolvedValue(application), getConversation: vi.fn().mockResolvedValue(conversation), addAttachment: vi.fn(), createManagerNotification: vi.fn()
    } as any;
    const agent = {
      run: vi.fn().mockResolvedValue({
        result: { ...validResult, reply: amountStageQuestion, currentStageResponse: "answer", leadCardPatch: {} },
        reply: amountStageQuestion, model: "workflow-model", promptVersion: "v1"
      }),
      answerWithKnowledge: vi.fn().mockResolvedValue({
        reply: "Для вас доступно:\nБез изъятия: от 50 000 сом до MAX_LIMIT_WITHOUT сом\nСо стоянкой: от 50 000 сом до MAX_LIMIT_PARK сом",
        answerFound: true, questionUnderstood: true, shouldUseReply: true,
        sourceKeys: ["faq_maximum_loan_range", "docx_0249"], requestScope: "new_loan", model: "knowledge-model"
      })
    } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "maximum", channel: "web-test", externalContactId: "contact", text: "чем больше, тем лучше", attachments: [], timestamp: new Date() });

    expect(output.reply).toContain("Без изъятия: от 50 000 сом до 600 000 сом");
    expect(output.reply).toContain("Со стоянкой: от 50 000 сом до 1 500 000 сом");
    expect(output.reply).not.toBe("Поняла.");
  });
  ...
  it("keeps only the server-calculated range when the knowledge model repeats a generic limit", async () => {
    const client = {
      isConfigured: vi.fn().mockReturnValue(true),
    ...
    const knowledgeReply = await new AgentTurnService(client).answerWithKnowledge({
      messages: [],
      facts: {
      ...
      settings: {},
      text: "сколько максимум дадите",
      workflowFollowUp: ""
    });
    const reply = replaceMaximumLimitPlaceholders(knowledgeReply!.reply, {
      vehicleValue: 2_000_000,
      residenceRegion: "Токмок",
      residenceCategory: "BISHKEK_CHUY"
    } as any, {});

    expect(reply).toBe("Для вас доступно:\nБез изъятия: от 50 000 сом до 600 000 сом\nСо стоянкой: от 50 000 сом до 1 000 000 сом");
  ...
  it("recognizes 'сколько сможете дать больше всего' as a maximum-loan question", () => {
    expect(isMaximumLoanKnowledgeQuestion("сколько сможете дать больше всего")).toBe(true);
  });

  it("does not save the vehicle value as a requested amount for 'сколько сможете дать больше всего'", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...validResult,
      loanQuestionKind: "none",
      reply: "Поняла.",
      leadCardPatch: { requestedAmount: 1_740_000 }
    }) } }] }) } as any;

    const output = await new AgentTurnService(client).run({
      messages: [{ author: "ai", body: "Какая сумма займа Вам необходима?", createdAt: "now" } as any],
      facts: { vehicleModel: "Camry", vehicleYear: 2022, vehicleValue: 1_740_000 } as any,
      settings: {}, text: "сколько сможете дать больше всего", attachments: []
    });
  ...
  it("extracts an Osh residence outside the active vehicle stage", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...validResult, reply: "Распознано.", leadCardPatch: {}
    }) } }] }) } as any;
    const output = await new AgentTurnService(client).run({
      messages: [{ author: "ai", body: "Подскажите, пожалуйста, ориентировочную стоимость автомобиля, модель и год выпуска автомобиля.", createdAt: "now" } as any],
      facts: {}, settings: {}, text: "Я из оша, мне нужен миллион", attachments: []
    });
  ...
  it("treats a stated amount next to 'дадите' as the requested loan, not a maximum-limit question", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...validResult,
      loanQuestionKind: "none",
      hasMoney: true,
  ...
  it("records an explicit requested amount without routing it to the maximum-limit FAQ", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...validResult, hasMoney: true, loanQuestionKind: "maximum_limit", leadCardPatch: { requestedAmount: 1_000_000 }
    }) } }] }) } as any;
    const output = await new AgentTurnService(client).run({
      messages: [], facts: {}, settings: {}, text: "мне нужен 1 млн", attachments: []
    });
  ...
  it("accepts an explicit correction after vehicle stage completion but ignores a vehicle fact copied only from history", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...validResult,
      hasMoney: true,
      leadCardPatch: { vehicleModel: "Corolla", vehicleValue: 800_000 }
    }) } }] }) } as any;
    const output = await new AgentTurnService(client).run({
      messages: [
        { author: "client", body: "У меня Corolla 2018 года", createdAt: "before" } as any,
        { author: "ai", body: "Пожалуйста, отправьте фото ID.", createdAt: "now" } as any
      ],
      facts: {
        vehicleModel: "Camry", vehicleYear: 2020, vehicleValue: 1_500_000,
        requestedAmount: 500_000, requestedProgram: "without_storage", residenceCategory: "BISHKEK_CHUY",
        stageCompletion: { vehicle: true }
      } as any,
      settings: {}, text: "Стоимость авто теперь 800 000 сом", attachments: []
    });

    expect(output.result?.leadCardPatch.vehicleValue).toBe(800_000);
    expect(output.result?.leadCardPatch.vehicleModel).toBe("Camry");
  });

  it("changes a booked visit through the server parser, not a model-written slot", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...validResult, leadCardPatch: { visitDate: "2026-09-18", visitTime: "11:00" }
    }) } }] }) } as any;
    const output = await new AgentTurnService(client).run({
      messages: [{ author: "ai", body: "Есть ли у Вас ещё вопросы?", createdAt: "now" } as any],
      facts: {
        vehicleModel: "Camry", vehicleYear: 2020, vehicleValue: 1_500_000,
        requestedAmount: 500_000, requestedProgram: "parking", residenceCategory: "BISHKEK_CHUY",
        documentsProvided: true, declinedCarPhoto: true, familyStatus: "single",
        visitDate: "2026-09-18", visitTime: "15:00"
      } as any,
      settings: { timezone: "Asia/Bishkek" }, text: "приеду всё-таки в 6", attachments: []
    });

    expect(output.result?.leadCardPatch.visitDate).toBe("2026-09-18");
    expect(output.result?.leadCardPatch.visitTime).toBe("18:00");
  });
  ...
  it("uses the semantic classifier for a colloquial before-marriage answer", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn()
      .mockResolvedValueOnce({ choices: [{ message: { content: JSON.stringify({ ...validResult, reply: "Поняла.", leadCardPatch: {} }) } }] })
      .mockResolvedValueOnce({ choices: [{ message: { content: JSON.stringify({ timing: "before_marriage" }) } }] })
    } as any;
    const output = await new AgentTurnService(client).run({
      messages: [{ author: "ai", body: "Подскажите, пожалуйста, автомобиль был приобретён до брака, во время брака или после развода?", createdAt: "now" } as any],
      facts: { familyStatus: "divorced" } as any,
      settings: {}, text: "наверное до ещё", attachments: []
    });

    expect(output.result?.leadCardPatch).toMatchObject({
      familyStatus: "divorced", vehicleBoughtDuringMarriage: false, vehiclePurchasedDuringMarriage: false
    });
  });
  ...
  it("routes knowledge from only the previous AI message and the current client message", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({
      model: "knowledge-router-model",
      choices: [{ message: { content: JSON.stringify({ lookup: true }) } }]
    }) } as any;
    const service = new AgentTurnService(client);

    await expect(service.shouldLookupKnowledge({
      messages: [{ author: "ai", body: "Подскажите модель автомобиля.", createdAt: "now" } as any],
      text: "А ставка какая?"
    })).resolves.toBe(true);
    ...
    expect(request.response_format).toEqual({
      type: "json_schema",
      json_schema: expect.objectContaining({ name: "knowledge_router_response" })
    });
    expect(request.messages[0].content).toContain("router базы знаний");
    expect(JSON.parse(request.messages[1].content)).toEqual({
      previousAssistantMessage: "Подскажите модель автомобиля.",
      currentMessage: "А ставка какая?"
    });
    expect(request.messages[1].content).not.toContain("knowledge");
  ...
  it("routes a maximum-loan request to knowledge without relying on the router model", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn() } as any;

    await expect(new AgentTurnService(client).shouldLookupKnowledge({ messages: [], text: "максимальная сумма нужна" })).resolves.toBe(true);
    expect(client.createChatCompletion).not.toHaveBeenCalled();
  });
  ...
  it("retries an unavailable knowledge router up to three times", async () => {
    ...
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn()
      ...
      .mockRejectedValueOnce(abortError)
      .mockResolvedValueOnce({ choices: [{ message: { content: JSON.stringify({ lookup: true }) } }] })
    } as any;

    await expect(new AgentTurnService(client).shouldLookupKnowledge({ messages: [], text: "Зачем он?" })).resolves.toBe(true);
  ...
  it("uses the configured knowledge model with the complete approved corpus", async () => {
    ...
    try {
      const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ model: "knowledge-test-model", choices: [{ message: { content: JSON.stringify({ reply: "Да, GPS устанавливаем.", answerFound: true, requestScope: "new_loan" }) } }] }) } as any;
      const service = new AgentTurnService(client);
      const output = await service.answerWithKnowledge({ messages: [], facts: {}, text: "датчики ставите?", workflowFollowUp: "Подскажите модель автомобиля.", isFirstClientMessage: true });

      expect(output).toMatchObject({ reply: "Да, GPS устанавливаем.", answerFound: true, requestScope: "new_loan", model: "knowledge-test-model" });
      const request = client.createChatCompletion.mock.calls[0][0];
      ...
      expect(request.structured_outputs).toBe(true);
      ...
      expect(context.isFirstClientMessage).toBe(true);
    } finally {
  ...
  it.each([
    ...
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply: "К сожалению, у меня нет достоверной информации по этому вопросу.", answerFound: false, questionUnderstood: true, sourceKeys: [], requestScope: "not_new_loan", contextualPolicyRelation: null
    }) } }] }) } as any;
    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [], facts: {}, settings: {}, text, workflowFollowUp: ""
    });
  ...
  it("evaluates knowledge on an explicit callback question even when workflow did not request it", async () => {
    const facts = { vehicleModel: "Camry", vehicleYear: 2022, vehicleValue: 3_000_000, requestedAmount: 600_000, requestedProgram: "parking", residenceRegion: "Бишкек", residenceCategory: "BISHKEK_CHUY" };
    ...
    const agent = {
      run: vi.fn().mockResolvedValue({
        result: { ...validResult, reply: "Есть ли у Вас ещё вопросы?", leadCardPatch: {}, needsKnowledgeLookup: false },
        reply: "Есть ли у Вас ещё вопросы?", model: "workflow-model", promptVersion: "v1"
      }),
      answerWithKnowledge: vi.fn().mockResolvedValue({ reply: "Обычно менеджер связывается с клиентами в течение часа.", answerFound: true, model: "knowledge-model" })
    } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "m", channel: "web-test", externalContactId: "c", text: "а когда позвонит менеджер", attachments: [], timestamp: new Date("2026-09-16T11:47:25.000Z") });
  ...
  it("returns only a standalone KB answer on the first message without starting a new-loan workflow", async () => {
    const stageQuestion = "Подскажите, пожалуйста, ориентировочную стоимость автомобиля.";
    const knowledgeReply = "Да, для наших клиентов есть чай и кофе.";
    const application = { id: "app", facts: {}, contactId: "contact", stage: "NEW", status: "need_more_data" } as any;
    const conversation = { id: "conversation", application, channel: "web-test", messages: [] } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application }),
    ...
    expect(agent.answerWithKnowledge).toHaveBeenCalledWith(expect.objectContaining({ isFirstClientMessage: true }));
  ...
  it.each([
    ["сколько дадите", "Максимальная сумма зависит от оценочной стоимости автомобиля."],
    ["какая ставка?", "Ставка определяется по программе займа после оценки автомобиля."]
  ])("returns only the KB answer for a first-message FAQ: %s", async (text, knowledgeReply) => {
    const stageQuestion = "Подскажите, пожалуйста, ориентировочную стоимость автомобиля.";
    const application = { id: "app", facts: {}, contactId: "contact", stage: "NEW", status: "need_more_data" } as any;
    const conversation = { id: "conversation", application, channel: "web-test", messages: [] } as any;
    const store = {
    ...
    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "faq", channel: "web-test", externalContactId: "contact", text, attachments: [], timestamp: new Date() });
  ...
  it("returns only the existing-contract redirect on the first message", async () => {
    const stageQuestion = "Подскажите, пожалуйста, ориентировочную стоимость автомобиля.";
    const redirect = "Я Айлин — виртуальный помощник по вопросам оформления новых займов. Если у Вас уже оформлен займ, пожалуйста, позвоните по телефону +996 502 108 108 или напишите в WhatsApp +996 776 108 108. Наши специалисты проверят информацию по Вашему договору и помогут решить Ваш вопрос.";
    const application = { id: "app", facts: {}, contactId: "contact", stage: "NEW", status: "need_more_data" } as any;
    const conversation = { id: "conversation", application, channel: "web-test", messages: [] } as any;
    ...
    const agent = {
      ...
      answerWithKnowledge: vi.fn().mockResolvedValue({ reply: redirect, answerFound: true, shouldUseReply: true, requestScope: "not_new_loan", model: "knowledge-model" })
    } as any;
  ...
  it("instructs the KB to distinguish standalone first-message questions from existing-loan servicing", () => {
    ...
    expect(prompt).toContain("Самостоятельный вопрос из `knowledge`");
    expect(prompt).toContain("не превращает диалог в заявку");
    expect(prompt).toContain("не по списку фраз");
    expect(prompt).toContain("семантически является ответом на активный этап");
    expect(prompt).toContain("нет права");
    expect(prompt).toContain("Не резюмируйте карточку и слова клиента");
    expect(prompt).toContain("Исключение для максимальной суммы");
    expect(prompt).toContain("чем больше, тем лучше");
  ...
  it("keeps a programme-stage answer when KB incorrectly adds an unasked rate", async () => {
    const programmeQuestion = "Вас интересует займ без изъятия автомобиля или с постановкой автомобиля на охраняемую стоянку?";
    const workflowReply = "Подскажите, пожалуйста, Вашу прописку.";
    const unsolicitedKnowledgeReply = "Ставка определяется индивидуально после осмотра автомобиля.";
    const application = {
      id: "app", contactId: "contact", stage: "ELIGIBILITY_CHECK", status: "need_more_data",
      facts: { vehicleModel: "Astra", vehicleYear: 2009, vehicleValue: 5_000_000, requestedAmount: 1_000_000 }
    } as any;
    const conversation = {
      id: "conversation", application, channel: "web-test",
      messages: [{ author: "ai", body: programmeQuestion, createdAt: "now" }]
    } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application }),
      addMessage: vi.fn().mockResolvedValue({ id: "message", author: "client", body: "", createdAt: "now" }),
    ...
    const agent = {
      run: vi.fn().mockResolvedValue({
        result: { ...validResult, reply: workflowReply, currentStageResponse: "answer", leadCardPatch: { requestedProgram: "without_storage" } },
        reply: workflowReply, model: "workflow-model", promptVersion: "v1"
      }),
      answerWithKnowledge: vi.fn().mockResolvedValue({ reply: unsolicitedKnowledgeReply, answerFound: true, shouldUseReply: true, requestScope: "new_loan", model: "knowledge-model" })
    } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "programme", channel: "web-test", externalContactId: "contact", text: "без изъятия", attachments: [], timestamp: new Date() });

    expect(agent.answerWithKnowledge).toHaveBeenCalledOnce();
    expect(output.reply).toContain(workflowReply);
    expect(output.reply).not.toContain("Ставка определяется");
  });
  ...
  it("never delivers a KB workflow sentinel for a new loan message containing needed amount", async () => {
    const application = { id: "app", facts: {}, contactId: "contact", stage: "NEW", status: "need_more_data" } as any;
    const conversation = { id: "conversation", application, channel: "web-test", messages: [] } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application }),
      addMessage: vi.fn().mockResolvedValue({ id: "message", author: "client", body: "", createdAt: "now" }),
      updateFacts: vi.fn().mockResolvedValue(["vehicleModel", "vehicleValue", "requestedAmount"]), saveAgentState: vi.fn(), getApplication: vi.fn().mockResolvedValue(application), getConversation: vi.fn().mockResolvedValue(conversation), addAttachment: vi.fn(), createManagerNotification: vi.fn()
    } as any;
    const workflowReply = "Вас интересует займ без изъятия автомобиля или с постановкой автомобиля на охраняемую стоянку?";
    const agent = {
      normalizeMoney: vi.fn().mockResolvedValue([
        { field: "requestedAmount", amount: 1_000_000, currency: "KGS", confidence: 0.99 },
        { field: "vehicleValue", amount: 3_000_000, currency: "KGS", confidence: 0.99 }
      ...
      run: vi.fn().mockResolvedValue({
        result: { ...validResult, reply: workflowReply, leadCardPatch: { vehicleMake: "Suzuki", vehicleModel: "Lifan" } },
        reply: workflowReply, model: "workflow-model", promptVersion: "v1"
      }),
      answerWithKnowledge: vi.fn().mockResolvedValue({ reply: "__WORKFLOW_STAGE_RESPONSE__", answerFound: false, shouldUseReply: true, model: "knowledge-model" })
    } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "m", channel: "web-test", externalContactId: "contact", text: "сузуки лифан нужно 1 слн стоит 3 млн примерно", attachments: [], timestamp: new Date() });

    expect(agent.answerWithKnowledge).toHaveBeenCalledOnce();
    expect(output.reply).toContain(workflowReply);
    expect(output.reply).not.toContain("__WORKFLOW_STAGE_RESPONSE__");
    expect(store.updateFacts).toHaveBeenCalledWith(application, expect.objectContaining({ vehicleValue: 3_000_000, requestedAmount: 1_000_000 }));
  });

  it("calls the KB model when the client answers the active price stage", async () => {
    const client = {
      isConfigured: vi.fn().mockReturnValue(true),
      createChatCompletion: vi.fn().mockResolvedValue({
        model: "knowledge-model",
        choices: [{ message: { content: JSON.stringify({ reply: "", answerFound: false, requestScope: "new_loan" }) } }]
      })
    } as any;
    const service = new AgentTurnService(client);

    await service.answerWithKnowledge({
      messages: [{ author: "ai", body: "Подскажите, пожалуйста, ориентировочную стоимость автомобиля.", createdAt: "now" } as any],
      facts: { vehicleModel: "Camry", vehicleYear: 2020 } as any,
      settings: {},
      text: "машина стоит пять миллионов",
      workflowFollowUp: "Подскажите, пожалуйста, Вашу прописку."
    });

    expect(client.createChatCompletion).toHaveBeenCalledOnce();
  });

  it("does not mark a stated needed amount as an explicit KB question", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
    ...
    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [], facts: {}, settings: {}, text: "сузуки лифан нужно 1 слн стоит 3 млн примерно", workflowFollowUp: ""
    });
  ...
  it("shows the KB fallback for an explicit question instead of resuming the application workflow", async () => {
    ...
    const fallback = "К сожалению, у меня нет достоверной информации по этому вопросу. Когда Вы приедете, сотрудники с удовольствием подскажут Вам.";
    const agent = {
      run: vi.fn().mockResolvedValue({
        result: { ...validResult, reply: "Подскажите, пожалуйста, ориентировочную стоимость автомобиля, модель и год выпуска автомобиля.", leadCardPatch: {}, needsKnowledgeLookup: false },
        reply: "Подскажите, пожалуйста, ориентировочную стоимость автомобиля, модель и год выпуска автомобиля.", model: "workflow-model", promptVersion: "v1"
      }),
      answerWithKnowledge: vi.fn().mockResolvedValue({ reply: fallback, answerFound: false, shouldUseReply: true, model: "server-knowledge-fallback" })
    } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "m", channel: "web-test", externalContactId: "c", text: "У меня машина сломалась, мне надо к вам заехать?", attachments: [], timestamp: new Date("2026-09-16T14:18:35.000Z") });

    expect(output.reply).toContain(fallback);
    expect(output.reply).not.toContain("ориентировочную стоимость автомобиля");
  });

  it("instructs KB to treat a broken client vehicle as existing-loan servicing by meaning", async () => {
    const redirect = "Я Айлин — виртуальный помощник по вопросам оформления новых займов. Если у Вас уже оформлен займ, пожалуйста, позвоните по телефону +996 502 108 108 или напишите в WhatsApp +996 776 108 108. Наши специалисты проверят информацию по Вашему договору и помогут решить Ваш вопрос.";
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({ reply: redirect, answerFound: true, questionUnderstood: true, sourceKeys: ["faq_existing_contract_redirect"], contextualPolicyRelation: null }) } }] }) } as any;

    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [], facts: {}, settings: {}, text: "У меня машина сломалась, мне надо к вам заехать?", workflowFollowUp: ""
    });

    expect(output).toMatchObject({ reply: redirect, answerFound: true });
    expect(client.createChatCompletion.mock.calls[0][0].messages[0].content).toContain("у меня машина сломалась");
    expect(client.createChatCompletion.mock.calls[0][0].messages[0].content).toContain("Строгое правило достоверности");
    const context = JSON.parse(client.createChatCompletion.mock.calls[0][0].messages[1].content);
    expect(context.knowledge.some((chunk: { section?: string }) => chunk.section === "3.18")).toBe(true);
  });

  it("calls KB for a pure answer to the programme-selection stage but keeps the workflow sentinel", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply: "__WORKFLOW_STAGE_RESPONSE__", answerFound: false, questionUnderstood: false, sourceKeys: [], requestScope: "new_loan", contextualPolicyRelation: null
    }) } }] }) } as any;
    ...
    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [{ author: "ai", body: "Вас интересует займ без изъятия автомобиля или с постановкой автомобиля на охраняемую стоянку?", createdAt: "now" } as any],
      facts: { vehicleModel: "Camry", vehicleYear: 2020, vehicleValue: 1_500_000, requestedAmount: 500_000 } as any,
      settings: {},
      text: "без изъятия",
      workflowFollowUp: "Вас интересует займ без изъятия автомобиля или с постановкой автомобиля на охраняемую стоянку?"
    });
  ...
  it.each(["Без изъятия", "Со стоянкой"])("blocks an unsolicited KB rate for a bare programme selection: %s", async (text) => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply: "По программе со стоянкой ставка составляет 2,4% в месяц.",
      answerFound: true,
      questionUnderstood: true,
      sourceKeys: ["faq_parking_rate"],
      requestScope: "new_loan",
      contextualPolicyRelation: null
    }) } }] }) } as any;

    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [], facts: {}, settings: {}, text, workflowFollowUp: "Вас интересует займ без изъятия автомобиля или с постановкой автомобиля на охраняемую стоянку?"
    });

    expect(output).toMatchObject({ reply: "__WORKFLOW_STAGE_RESPONSE__", answerFound: false, shouldUseReply: false });
  });

  it("turns an unasked lead-card summary into the workflow sentinel", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply: "Ваш автомобиль Opel Astra 2009 года выпуска стоит ориентировочно 5 миллионов сом.",
      answerFound: true,
      questionUnderstood: true,
      sourceKeys: ["lead_card"],
      requestScope: "new_loan",
    ...
    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [{ author: "ai", body: "Подскажите, пожалуйста, ориентировочную стоимость автомобиля.", createdAt: "now" } as any],
      facts: { vehicleMake: "Opel", vehicleModel: "Astra", vehicleYear: 2009 } as any,
      settings: {},
      text: "стоит где-то 5 мил а модель Астра 2009",
      workflowFollowUp: "Вас интересует займ без изъятия автомобиля или с постановкой автомобиля на охраняемую стоянку?"
    });
  ...
  it("does not require a question mark for a KB-understood service request", async () => {
    const redirect = "Я Айлин — виртуальный помощник по вопросам оформления новых займов. Если у Вас уже оформлен займ, пожалуйста, позвоните по телефону +996 502 108 108 или напишите в WhatsApp +996 776 108 108. Наши специалисты проверят информацию по Вашему договору и помогут решить Ваш вопрос.";
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({ reply: redirect, answerFound: true, questionUnderstood: true }) } }] }) } as any;

    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [], facts: {}, settings: {}, text: "А у меня авто сломана", workflowFollowUp: "Подскажите стоимость автомобиля."
    });

    expect(output).toMatchObject({ reply: redirect, answerFound: true, shouldUseReply: true });
  });

  it("keeps an unknown-service fallback for 'Есть мастер по ремонту у вас' instead of converting it to a workflow sentinel", async () => {
    const fallback = "К сожалению, у меня нет достоверной информации по этому вопросу. Когда Вы приедете, сотрудники с удовольствием подскажут Вам.";
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply: fallback, answerFound: false, questionUnderstood: false, sourceKeys: [], requestScope: "not_new_loan", contextualPolicyRelation: null
    }) } }] }) } as any;

    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [{ author: "ai", body: "Пожалуйста, отправьте 2–3 фотографии автомобиля.", createdAt: "now" } as any],
      facts: {}, settings: {}, text: "Есть мастер по ремонту у вас", workflowFollowUp: "Пожалуйста, отправьте 2–3 фотографии автомобиля."
    });

    expect(output).toMatchObject({ reply: fallback, answerFound: false, questionUnderstood: true, shouldUseReply: true });
  });

  it("keeps an unsupported office-goods fallback instead of turning it into an address or workflow sentinel", async () => {
    const fallback = "К сожалению, у меня нет достоверной информации по этому вопросу. Когда Вы приедете, сотрудники с удовольствием подскажут Вам.";
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply: fallback,
      answerFound: true,
    ...
    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [], facts: {}, settings: {}, text: "У вас есть водка в офисе", workflowFollowUp: ""
    });

    expect(output).toMatchObject({ reply: fallback, answerFound: false, questionUnderstood: true, shouldUseReply: true });
    expect(output?.reply).not.toContain("Молодой Гвардии");
  });

  it("delivers a KB fallback for a natural service request even if the workflow model calls it a stage answer", async () => {
    const fallback = "К сожалению, у меня нет достоверной информации по этому вопросу. Когда Вы приедете, сотрудники с удовольствием подскажут Вам.";
    const stageQuestion = "Пожалуйста, отправьте 2–3 фотографии автомобиля.";
    const application = { id: "app", contactId: "contact", stage: "COLLECTING_DOCUMENTS", status: "need_more_data", facts: {} } as any;
    const conversation = { id: "conversation", application, channel: "web-test", messages: [{ author: "ai", body: stageQuestion, createdAt: "now" }] } as any;
    const store = {
      getOrCreateConversation: vi.fn().mockResolvedValue({ conversation, application }),
      addMessage: vi.fn().mockResolvedValue({ id: "message", author: "client", body: "Есть мастер по ремонту у вас", createdAt: "now" }),
      updateFacts: vi.fn().mockResolvedValue([]), saveAgentState: vi.fn(), getApplication: vi.fn().mockResolvedValue(application), getConversation: vi.fn().mockResolvedValue(conversation), addAttachment: vi.fn(), createManagerNotification: vi.fn()
    } as any;
    const agent = {
      run: vi.fn().mockResolvedValue({ result: { ...validResult, reply: stageQuestion, currentStageResponse: "answer", leadCardPatch: {} }, reply: stageQuestion, model: "workflow-model", promptVersion: "v1" }),
      answerWithKnowledge: vi.fn().mockResolvedValue({ reply: fallback, answerFound: false, questionUnderstood: true, shouldUseReply: true, requestScope: "not_new_loan", model: "knowledge-model" })
    } as any;

    const output = await new DialogueOrchestratorService(agent, store, { getValues: vi.fn().mockResolvedValue({}) } as any, { log: vi.fn() } as any)
      .receive({ externalMessageId: "repair", channel: "web-test", externalContactId: "contact", text: "Есть мастер по ремонту у вас", attachments: [], timestamp: new Date() });

    expect(output.reply).toContain(fallback);
    expect(output.reply).not.toContain(stageQuestion);
  });

  it("does not let KB answer an application fact", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply: "__WORKFLOW_STAGE_RESPONSE__", answerFound: false, questionUnderstood: false, sourceKeys: [], requestScope: "new_loan", contextualPolicyRelation: null
    }) } }] }) } as any;

    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [], facts: {}, settings: {}, text: "Привет, нужен займ без изъятия", workflowFollowUp: "Подскажите модель автомобиля."
    });

    expect(output).toMatchObject({ answerFound: false, shouldUseReply: false });
    expect(output?.reply).toBe("__WORKFLOW_STAGE_RESPONSE__");
    expect(client.createChatCompletion.mock.calls[0][0].messages[0].content).toContain("это не вопросы к базе знаний");
  });

  it("rejects a region-10 KB refusal when the client only supplied vehicle facts", async () => {
    const forbiddenReply = "К сожалению, под такой автомобиль займ не выдаётся. Если у вас есть другой автомобиль без региона 10, можно продолжить оформление по нему.";
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply: forbiddenReply, answerFound: true, questionUnderstood: true, sourceKeys: ["docx_0381"], requestScope: "new_loan", contextualPolicyRelation: null
    }) } }] }) } as any;

    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [], facts: {}, settings: {}, text: "Кио рио 2020 пять миллионов", workflowFollowUp: vehicleStageQuestion
    });

    expect(output).toMatchObject({
      reply: "__WORKFLOW_STAGE_RESPONSE__", answerFound: false, shouldUseReply: false
    });
  });

  it("accepts a region-10 KB answer when the region word contains a typo", async () => {
    const reply = "К сожалению, под такой автомобиль займ не выдаётся.";
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply, answerFound: true, questionUnderstood: true, sourceKeys: ["docx_0381"], requestScope: "new_loan", contextualPolicyRelation: null
    }) } }] }) } as any;

    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [], facts: {}, settings: {}, text: "у меня 10 реон", workflowFollowUp: vehicleStageQuestion
    });

    expect(output).toMatchObject({ reply, answerFound: true, shouldUseReply: true });
  });

  it("uses the preceding client fact when an earlier KB reply was unhelpful", async () => {
    const reply = "К сожалению, под автомобиль с регионом 10 мы займ не выдаём. Если у Вас есть другой автомобиль без региона 10, можем продолжить оформление по нему.";
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply, answerFound: true, questionUnderstood: true, sourceKeys: ["docx_0381"], contextualPolicyRelation: "follow_up"
    }) } }] }) } as any;

    const output = await new AgentTurnService(client).answerWithKnowledge({
      messages: [
        { author: "client", body: "а у меня авто из 10 рег", createdAt: "first" } as any,
        { author: "ai", body: "К сожалению, у меня нет достоверной информации по этому вопросу.", createdAt: "second" } as any
      ],
      facts: {}, settings: {}, text: "и что делать", workflowFollowUp: ""
    });

    expect(output).toMatchObject({ reply, answerFound: true });
    const context = JSON.parse(client.createChatCompletion.mock.calls[0][0].messages[1].content);
    expect(context.history).toEqual(expect.arrayContaining([expect.objectContaining({ author: "client", text: "а у меня авто из 10 рег" })]));
  });

  it("allows a polite contextual response to an insult", async () => {
    const reply = "Понимаю, что ситуация может раздражать. Напишите, пожалуйста, что именно нужно уточнить по займу или автомобилю.";
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      reply, answerFound: true, questionUnderstood: true, sourceKeys: ["conversation_context"], contextualPolicyRelation: null
    }) } }] }) } as any;

    const output = await new AgentTurnService(client).answerWithKnowledge({ messages: [], facts: {}, settings: {}, text: "ты тупая ?", workflowFollowUp: "" });
  ...
  it("retries an invalid KB response once and returns the server fallback", async () => {
    ...
    const output = await new AgentTurnService(client, logs).answerWithKnowledge({
    ...
    expect(logs.warn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        metadata: expect.objectContaining({
          ...
          responseChoice: { finish_reason: "stop", message: { content: null, refusal: "refused by provider" } },
          routerAiResponse: {
            model: "openai/gpt-4o-mini",
            choices: [{ finish_reason: "stop", message: { content: null, refusal: "refused by provider" } }]
          }
  ...
  it("applies the server limit from limitChoice without a model-written amount or programme", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...validResult,
      reply: "Распознано.",
      limitChoice: "keep_car",
      leadCardPatch: {}
    }) } }] }) } as any;
    const output = await new AgentTurnService(client).run({
  ...
  it("never invents a time when a client names only a possible visit day", async () => {
    ...
    try {
      const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
        ...validResult, reply: "Запишу на понедельник в 15:00.", leadCardPatch: { visitRequested: true, visitDate: "2026-09-14", visitTime: "15:00" }
      }) } }] }) } as any;
      const output = await new AgentTurnService(client).run({
        messages: [{ author: "ai", body: "Офис работает с понедельника по пятницу с 11:00 до 19:00. Для оформления нужно приехать не позднее 18:00. На какой день и время Вам удобно подъехать?", createdAt: "now" } as any],
        facts: {
        ...
        settings: {}, text: "не знаю, может в понедельник смогу", attachments: []
      });

      expect(output.result?.leadCardPatch).toEqual(expect.objectContaining({ visitRequested: true, visitDate: "2026-09-14" }));
      expect(output.result?.leadCardPatch.visitTime).toBeUndefined();
      expect(output.reply).toMatch(/в какое время вам удобно подъехать/i);
      expect(output.reply).not.toMatch(/15:00/);
    } finally {
      vi.useRealTimers();
    }
  });

  it("confirms a corrected time after an already booked visit", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...validResult, reply: "Поняла.", leadCardPatch: { visitTime: "15:00" }
    }) } }] }) } as any;
    const output = await new AgentTurnService(client).run({
      ...
      settings: {}, text: "в 3 неудобно, давайте в 6", attachments: []
    });
  ...
  it.each(["потом напишу", "вернусь позже", "давайте продолжим потом", "сейчас не могу"])("pauses the application without a follow-up question for %s", async (text) => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...
      reply: "Распознано.",
      currentStageResponse: "unrelated",
      leadCardPatch: { clientPaused: true },
      dialogueState: { stage: "PAUSED", status: "target_reached", nextAction: "pause" }
    }) } }] }) } as any;
    const output = await new AgentTurnService(client).run({
      messages: [{ author: "ai", body: "Пожалуйста, отправьте фото ID и свидетельства о регистрации автомобиля с обеих сторон.", createdAt: "now" } as any],
      facts: { vehicleModel: "Camry", vehicleYear: 2022, vehicleValue: 1_000_000, requestedAmount: 300_000, requestedProgram: "parking", residenceRegion: "Бишкек", residenceCategory: "BISHKEK_CHUY" } as any,
      settings: {}, text, attachments: []
    });
  ...
  it("resumes a paused application when the client supplies a new application fact", async () => {
    const client = { isConfigured: vi.fn().mockReturnValue(true), createChatCompletion: vi.fn().mockResolvedValue({ choices: [{ message: { content: JSON.stringify({
      ...validResult,
      reply: "Распознано.",
      currentStageResponse: "answer",
      leadCardPatch: { clientPaused: false, requestedProgram: "parking" }
    }) } }] }) } as any;
    const output = await new AgentTurnService(client).run({
      messages: [{ author: "ai", body: "Вас интересует займ без изъятия автомобиля или с постановкой автомобиля на охраняемую стоянку?", createdAt: "now" } as any],
      facts: { clientPaused: true, vehicleModel: "Camry", vehicleYear: 2022, vehicleValue: 1_000_000, requestedAmount: 300_000, residenceRegion: "Бишкек", residenceCategory: "BISHKEK_CHUY" } as any,
      settings: {}, text: "со стоянкой", attachments: []
    });
  ...
  it("keeps older-vehicle policy and client-fact echoing out of agent prompts", () => {
    ...
    expect(agentPrompt).toContain("не формируете клиентский текст");
  });

  it("instructs the main agent to extract off-stage city, programme, price and loan facts", () => {
    const agentPrompt = readFileSync(new URL("../ai/prompts/agent.system.md", import.meta.url), "utf8");

    expect(agentPrompt).toContain("Независимый проход по фактам текущей реплики");
    expect(agentPrompt).toContain("hasMoney=true");
    expect(agentPrompt).toContain("Я из Оша, мне нужен миллион");
  });
