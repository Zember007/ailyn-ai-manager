...
function buildSpecialAnswers(
  ...
  if (effectiveFamilyStatus === "divorced" && facts.vehicleBoughtDuringMarriage === undefined) {
    answers.push({
      ...
      text: "Нотариальное согласие бывшего супруга или супруги не требуется. Подскажите, пожалуйста, автомобиль был приобретён до брака, во время брака или после развода?",
      exact: true
    });
  }
...
function clarificationQuestion(
  ...
  const clarificationByFact: Record<string, string> = {
    ...
    vehicleBoughtDuringMarriage: "Я не до конца поняла, когда был куплен автомобиль. Уточните, пожалуйста, автомобиль был приобретён до брака, во время брака или после развода?",
