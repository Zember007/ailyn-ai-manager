...
Work only from the current `main` branch. Before making changes, confirm local `main` is checked out and synchronized with `origin/main`. Do not implement or deliver fixes from feature branches unless the user explicitly requests that branch.
...
- `pnpm typecheck`
- `pnpm build`
- local Docker workflow smoke test with relevant services up
- container and application log review for each changed workflow
...
2. Run the affected workflow locally in Docker before reporting completion whenever the workflow depends on app-to-app or infrastructure interaction.
3. Review container and application logs during the local Docker run, and if any workflow fails or behaves unexpectedly, continue debugging and fixing before moving on.
4. Commit the finished work to git with a clear non-interactive commit message.
5. Push the commit to GitHub.
6. Check that CI and deployment complete successfully after the push. If they fail, continue until the failure is understood and either fixed or clearly reported.
